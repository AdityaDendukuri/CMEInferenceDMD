...
"""
Convert state indices to actual molecule counts
This function corrects the state representation to reflect actual molecular counts
rather than grid indices
"""
function state_idx_to_molecule_counts(state_indices, species_names)
    # State indices in the grid are 1-based (1 index = 0 molecules)
    # So we need to subtract 1 to get the actual molecule counts
    molecule_counts = [idx - 1 for idx in state_indices]
...
"""
Enhanced simplified spectral analysis with fixed visualizations
"""
function simplified_spectral_analysis_with_plots(G, λ, Φ, selected_states, species_names, grouped_reactions, stoich_stats)
    ...
    # Print the table using our custom function instead of UnicodePlots.table
...
"""
Enhanced full spectral analysis with fixed visualizations
"""
function full_spectral_analysis_with_plots(G, λ, Φ, grouped_reactions, stoich_stats, selected_states, species_names)
    println("\n==== Enhanced Spectral Analysis with Visualizations ====")
    
    # Replace use of UnicodePlots.table with our custom print_simple_table
    # Rest of the function remains the same as before, but using our fixed visualization functions
    
    # ...implementation continues...
    
    # When using tables, replace:
    # println(UnicodePlots.table(rows, headers))
    # with:
    # print_simple_table(rows, headers)
...
"""
Main analysis function combining both visualization approaches - Fixed version
"""
function analyze_and_select_reactions_with_plots(G, λ, Φ, grouped_reactions, stoich_stats, selected_states, species_names)
    println("\n==== Starting Spectral Analysis with Visualizations ====")
    
    # Just use the simplified version which we've fixed
    return simplified_spectral_analysis_with_plots(G, λ, Φ, selected_states, species_names, grouped_reactions, stoich_stats)
end
...
function infer_crn_with_catalyst_dmd(max_dim=1000)
  ...
  selected_reactions, spectral_scores = analyze_and_select_reactions_with_plots(
      G, λ, Φ, grouped_reactions, stoich_stats, selected_states, species_names
  )

  # Add selected reactions to result dictionary
  #result["spectral_selected_reactions"] = selected_reactions
